...
def test_products_main(chromeBrowser):

    chromeBrowser.get('https://admin-dev.goopter.com/products/')
    

def test_products_searchbox_type(chromeBrowser):
    sleep(2)
...
#Actions
def test_add_group_purchase_product(chromeBrowser):
...
def test_products_threedots(chromeBrowser):
...
def test_products_sharing(chromeBrowser):
...
def test_product_enable(chromeBrowser):
